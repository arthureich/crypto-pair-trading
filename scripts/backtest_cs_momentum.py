#!/usr/bin/env python3
"""Run the pre-registered cross-sectional momentum backtest (TASK-CS-001).

A single, fixed hypothesis: weekly cross-sectional crypto momentum
(Liu & Tsyvinski 2021 style, faithfully replicated), gate evaluated ONLY
on the out-of-sample period (2025-06 through 2026-05, the exact boundary
already pre-registered in TASK-TSREV-001, reused here without re-choosing
it). No sweep, no secondary cells -- see
docs/pre_registers/TASK-CS-001.md and project_control/DECISIONS.md
ADR-0017.
"""
...
from src.research.cs_momentum import (  # noqa: E402
...
OUTPUT_JSON = (
    PROJECT_ROOT / "data/research/binance_public/cost_pilot/cs_momentum_backtest_results.json"
)
REPORT_MD = PROJECT_ROOT / "reports/cs_momentum_backtest_final.md"
...
def main() -> int:
    ...
    trades = run_cross_sectional_momentum_backtest(bars, config)
    ...
    oos_summary = summarize_cross_sectional_momentum(oos_trades, config, baseline_dd_bps)
    is_summary = summarize_cross_sectional_momentum(is_trades, config)
    full_summary = summarize_cross_sectional_momentum(trades, config)
    ...
    print(f"GATE (TASK-CS-001, weekly momentum, OOS only): {gate_decision}", file=sys.stderr)
...
def _write_report(payload: dict[str, Any]) -> None:
    lines = [
        ...
        "`docs/pre_registers/TASK-CS-001.md` -- a faithful replication of "
        "weekly cross-sectional crypto momentum (Liu & Tsyvinski 2021 "
        "style), not an internally-invented signal. Gate is decided ONLY "
