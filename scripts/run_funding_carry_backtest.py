#!/usr/bin/env python3
"""Run the pre-registered funding-rate carry backtest (TASK-FUND-002).

Implements the hypothesis pre-registered in
`tasks/funding_carry/TASK-FUND-001-define-hypothesis.md`: cross-sectional
funding-rate carry, ranked by real `funding_rate_asof`, on the same 20
symbols / 2023-06 through 2026-05 window already normalized for Sprint 7 --
no new data download. Runs the PRIMARY configuration (K=5) plus two
descriptive-only secondary configurations (K=3, K=8); the gate decision
depends solely on K=5, per the pre-registered rule (no ex-post K selection).
"""
...
def main() -> int:
    ...
    for k in (PRIMARY_K, *SECONDARY_KS):
        ...
        variants[k] = {"config": asdict(config), "summary": asdict(summary), "results": results}
    ...
    payload = {
        ...
        "variants": {
            str(k): {"config": v["config"], "summary": v["summary"]} for k, v in variants.items()
        },
    }
            ...
            rows.append(
                {
                    ...
                    "eligible_symbol_count": result.eligible_symbol_count,
                    "long_symbols": "|".join(result.long_symbols),
                    "short_symbols": "|".join(result.short_symbols),
