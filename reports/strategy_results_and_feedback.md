...
| VRP-timing (ALT-012, BTC/ETH weekly) | long/short sign(vrp_z), 7d rebalance, 6bps/leg, vs buy-hold | Sharpe>buyhold AND net>0 AND maxDD<=buyhold AND sub-period-consistent | Sharpe **0.493 vs 0.349** (better), net better, but maxDD **0.787 vs 0.774 (worse)** and inconsistent (neg sub-period 2024-25) | NAO_PASSA (dev) | The VRP signal is real (ALT-011 info + economics), but a naive weekly long/short standalone is modest & drawdown-heavy -- the 197 bps decile spread was overlapping-sample-inflated. Long-only secondary looked better (Sharpe 0.601, maxDD 0.500) but is descriptive-only. VRP as feature/overlay also tested (ALT-013) and REJECTED (see next row) |
| VRP-as-overlay on TSM (ALT-013, equal-risk blend) | blend TSM (trend) + VRP-timing (BTC/ETH) return streams | blend Sharpe > TSM AND consistent AND maxDD not worse | blend **0.966 vs TSM 0.991** (worse); VRP sleeve Sharpe 0.49, corr +0.18 | NAO_PASSA (dev) | The equal-risk blend of a strong stream (TSM 0.99) with a weak, POSITIVELY-correlated one (VRP 0.49, corr +0.18) drags Sharpe down; VRP sleeve is -0.48 in 2024-25. VRP doesn't help the perp book as an overlay either. **Closes the FREE VRP exploration**: real signal, no viable free monetization (not standalone, not overlay). Remaining VRP avenues (skew/surface, Angle-A options book) are user spend/instrument decisions |
| BASIS-001 -- cash-and-carry DELTA-NEUTRAL (spot x dated future, BTC/ETH) | ADR-0034 (NEW family, not TSM). Buy spot + short same-asset quarterly future at ~90d lead, hold to expiry, capture basis convergence net of conservative cost (26 bps round-trip); 16 contracts (BTC/ETH x 8 quarterlies 2024-25); Binance only | net+ on Binance AND Bybit AND OKX; DD << TSM; delta~0; PnL not concentrated; no high leverage; majors capacity; ROC justifies risk | net APR **mean 11.3% / median 8.7%, 100% positive (16/16)**; rolled **maxDD 0.0%**; **delta-corr -0.14** (~0); worst adverse MTM <=1.6%; basis 32% APR (mid-2024) -> ~3-5% (2025) | **PAUSED -- real but currently compressed** | First delta-neutral result and a very different profile from the TSM: LOW directional risk and LOW path-dependence (locked return = entry basis, proven in tests) -- steady positive carry -- PROVIDED both legs stay operational and adequately margined. NOT risk-free / not literally "no drawdown" (funding can invert, spot-perp spread can widen, legs can be liquidated separately, a leg can fail; plus exchange/index/ADL/custody risk). Attacks exactly what failed the TSM (31-58% DD, directional, concentrated). Basis is COMPRESSING over time (post-ETF), so the edge is thinning. NOT approved: locked criterion #1 requires net-positive on Binance AND Bybit AND OKX -- this is Binance-only (cross-exchange is research step #4). Real risks stand: margin/liquidation of the short in extreme moves, exchange/custody, conservative-constant costs (not real fills), fixed-lead entry (no timing). Separate modules (`cash_carry.py`, `basis_data.py`); TSM infra untouched. **Cross-exchange (criterion #1)**: dated cross-exchange is DATA-GATED (OKX/Bybit drop expired contracts) -> pivoted to spot x PERP funding-neutral carry (same delta-neutral family). Binance BTC/ETH 7.2%/7.5% and Bybit 7.5%/7.5% net APR at full ~3y depth, near-identical -> NOT a single-venue artifact (strong 2-venue confirmation); OKX free funding-history is depth-limited (~3mo, +1.2% recent). Common recent window: all 3 ~0 -> the carry has COMPRESSED to ~0 in 2026 (edge is real but regime-dependent; 7.5% is from 2023-2024). Criterion #1 PARTIALLY met (Binance+Bybit strong; OKX inconclusive by data limit). **(b) Time-profile characterization** (fixed windows, a-priori ~11% operational hurdle NOT fit to pass): return on TOTAL capital (Binance BTC) 2023H2 7.2% -> 2024 10.6% (peak) -> 2025 4.4% -> 2026 0.5%; NEVER cleared the full ~11% hurdle in any calendar window (2024 just under); crossed below a lean ~5% hurdle around 2025. So the carry stopped compensating the operational/counterparty risks ~2025 and is ~0 now. **FAMILY PAUSED = real but currently compressed**; re-open only if forward funding rises back above the hurdle for a sustained run. NOT continuing to calendar spread (same basis premium, correlated). `reports/basis_perp_carry_xexch.md`, `reports/basis_carry_characterization.md` |
...
| TSM-011 -- CROSS-EXCHANGE generalization (Bybit) | FIXED base TSM (zero re-tune), same 20 symbols, same code, on Bybit v5 (klines+funding) vs Binance | **Bybit Sharpe 0.971 vs Binance 0.970**; maxDD 0.341 vs 0.347; turnover/net/sub-periods near-identical | **GENERALIZES (venue-independent)** | Same fixed strategy on a different exchange gives near-identical results -> the edge is NOT a Binance data/microstructure artifact. CAVEAT: same-symbol prices are arbitraged across venues, so similarity is expected -- this rules out venue-data artifacts; the cross-UNIVERSE tests remain the stronger diversity evidence. The near-identical match also validates the Bybit pipeline correctness. (OKX = follow-up; Hyperliquid dropped -- shallow history.) |
| TSM-012 -- CROSS-ASSET-CLASS generalization | SAME economic horizons (28d/7d/5d) in DAILY bars on indices/commodities/forex/ETFs (Yahoo, no funding), zero re-tune | TSM beats buy-hold in **0/4** classes; absolute Sharpe weakly + in commodities (0.18) / ETFs (0.13), - in indices (-0.29) / forex (-0.16) | **DOCUMENTED LIMIT (not a refutation)** | By the pre-registered "beats buy-hold" bar the TSM does NOT generalize to TradFi on this window. Literature check (rule #6): 2023-2024 was a DOCUMENTED weak period for TradFi trend/CTAs (equity bull, low trendiness), and trend's role is crisis-alpha/diversification, NOT beating a bull -- so the bar is adverse-by-construction and the window a known weak trend regime. Honest read: the TSM's strong edge is CRYPTO-SPECIFIC on this window (crypto had high dispersion/vol/negative-alt-drift = ideal for long/short trend; TradFi was a smooth bull = adverse). BOUNDS the claim (crypto multi-universe + cross-exchange robust; cross-asset NOT established). No strategy change (regime/benchmark effect). Fair future test: longer window + large pooled universe + diversification bar |
| TSM-013 -- STATISTICAL characterization (cross-universe synthesis) | descriptive aggregation of committed artifacts (TSM-008/009/010/011/012); reads JSONs only, NO backtest/param change; crypto (in-domain, n=7) vs TradFi (out-of-domain, n=4) never pooled; bootstrap CI seed=0 | CRYPTO base Sharpe **mean 0.783** (median 0.832, sd 0.220, CV 0.28), **bootstrap 95% CI [0.628, 0.928] EXCLUDES zero** (t-CI [0.579, 0.987]); positive **7/7**, beats buy-hold **7/7**; overlay combined>base only **3/7**; TradFi mean -0.037, 0/4 | **STATISTICALLY ROBUST IN-DOMAIN (crypto)** | Turns the whole validation set into one statement: the base vol-targeted TSM is a stable cross-universe property of crypto perps, not a lucky draw (CI excludes zero, 100% positivity, low CV 0.28). Degradation graceful (best gaming 1.00, worst old_guard 0.46, still positive -- no cliff). Overlays confirmed partly universe-specific (help in minority 3/7) -> base is the robust CORE. TradFi documented SEPARATELY as the honest LIMIT (never pooled). Caveats: n=7 small (CI wide-ish); temporal sub-period coverage only 1/7 crypto (original_20) -- honestly flagged. Descriptive only; no promotion, no param change |
| TSM-014 -- TEMPORAL robustness (a-priori windows) | reconstruct base-TSM PnL streams OFFLINE from cached bars (7 crypto universes); windows FIXED before results; (A) per-universe x 3 fixed sub-periods, (B) rolling W6=37/W12=73, (C) drawdown duration | (A) every universe positive in **>=2/3** sub-periods (weakest 2024-06/2025-05, mean 0.50, still 100% pos; only 1 near-zero negative cell defi -0.02); (B) pooled rolling **W6 77% / W12 91%** windows positive; (C) worst DD duration **~415d (~14mo)**, time-underwater 79-89% | **TEMPORALLY ROBUST (with an honest cost)** | The edge holds across TIME, not one hot window: positive across all 3 fixed sub-periods (backfills TSM-013 coverage 1/7 -> 7/7) and the large majority of rolling windows. HONEST COST (not sugar-coated): drawdowns are long -- up to ~14 months peak-to-recovery in the mid/alt universes (mid_alt_l1/defi/mid_tier_ref, the same ones weakest in the FINAL year, likely still underwater at window-end) and high time-underwater -- but at MODEST depth (maxDD ~0.31-0.80), i.e. shallow-but-long grinds typical of trend-following. Robustness verdict rests on (A)+(B); (C) is the honest risk. Offline, fixed params, a-priori windows; descriptive, no promotion. Fixed a real _sharpe epsilon bug (flat slice -> None) |
| TSM-015 -- LIQUIDITY-stress (tiers + cost sweep) | segment 45 cached survivors into liquidity TERCILES by median daily dollar-volume; FIXED base TSM per tier under cost sweep 6/12/20/30bps; OFFLINE (no download) | LOW tier Sharpe **0.842@6bps -> 0.714@30bps** (buy-hold **-0.510**); HIGH 0.769, MID **0.934**, LOW 0.842 @6bps (HIGH-LOW gap **-0.073**, no monotone degradation); LOW beats buy-hold at ALL costs incl. 20bps bar (0.767) | **LIQUIDITY-ROBUST (within survivors)** | The edge SURVIVES at the lower-liquidity end even at realistic cost -- the one discriminating in-domain axis left, and it holds. Strikingly HIGH is the LOWEST tier and MID the highest -> trend edge is strongest in the less-efficient alts, not majors (coherent with the whole thesis). HONEST COST: the LOW tier carries DEEPER drawdowns (maxDD **0.686** vs ~0.47 HIGH/MID) -- Sharpe comparable but risk higher in low-liq. SURVIVORSHIP CAVEAT (central, not hidden): truly illiquid/microcap perps lack 3y history and are ABSENT -> this tests only the lower-liq END of survivors, an OPTIMISTIC lower bound, not the real microcap tail; quote_volume is a coarse proxy (no spread/book depth). Descriptive; no promotion |
...
| **F -- Options** | **HIT + clears cost (LEAD)** | ALT-011 DVOL/VRP-as-predictor (free Deribit DVOL, zero cost): **`vrp_z@7d` TEM_INFORMACAO** -- variance risk premium (IV2-RV2) predicts 7d forward return, rho +0.087, POSITIVE and sign-consistent in ALL 3 sub-periods, theory-coherent (matches crypto-VRP literature). **First external hit.** Economic check: top-vs-bottom-decile gross spread **196.8 bps/7d vs ~12 bps cost** (monotonicity +0.67) -> **first signal to clear BOTH the info bar AND the gross-economics bar** (unlike FC-II-003's 1-2 bps). Caveats: tail-driven (not cleanly monotone), overlapping daily-sampled holds (overstates a real weekly strategy), in-sample, BTC/ETH-only. A LEAD, not a validated edge. ALT-012 ran the proper weekly backtest: naive long/short standalone NAO_PASSA (drawdown-heavy, inconsistent) -- the decile spread was overlapping-sample-inflated. VRP-as-overlay on TSM (ALT-013) also REJECTED (blend 0.966 < TSM 0.991; too weak + positively correlated). FREE VRP exploration closed: real signal, no free monetization. Skew/surface RECONNAISSANCE (2026-07-13): free Deribit API serves only the CURRENT option-chain snapshot + sparse trades + the ATM DVOL already used -> historical skew/25d-RR/butterfly/term-structure NOT reconstructible free -> PAID-data-gated (Tardis/Amberdata). VRP-harvesting (Angle A) still a user decision |
