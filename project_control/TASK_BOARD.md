...
| TASK-009-01 | 9 | Implementar fill_model.py | Backtest Agent | IN_REVIEW | - | QA / Chaos Testing Agent + Execution / Risk Agent | nenhum |
| TASK-009-02 | 9 | Implementar execution_simulator.py | Backtest Agent | IN_REVIEW | - | Quant Research Agent + QA / Chaos Testing Agent | nenhum |
| TASK-009-03 | 9 | Implementar replay_engine.py | Backtest Agent | IN_REVIEW | - | QA / Chaos Testing Agent + Market Data Agent | nenhum |
| TASK-009-04 | 9 | Testes de Sprint 9 (chaos, causalidade) | QA / Chaos Testing Agent | IN_REVIEW | - | Backtest Agent + PM Agent | nenhum |
| TASK-009-05 | 9 | Rodar replay real nos 13 pares aprovados | Backtest Agent | IN_PROGRESS | - | PM Agent + Quant Research Agent | rodando em background |
| TASK-009-06 | 9 | Relatorio e gate Sprint 9 | Documentation Agent | BLOCKED | - | PM Agent + Backtest Agent | depende de TASK-009-05 |
...
| TASK-009-01 | 90% | `src/backtest/fill_model.py` implemented: MARKET/IOC and LIMIT+TTL fills against level-1 quotes only, reusing `estimate_slippage`; latency via earliest-reachable-quote selection; ACK_UNKNOWN via deterministic hash of order_id, integrated with `AckGuardOrderStatus`. 11 tests passed. Found and fixed a real bug during chaos testing: `_coerce_side` did not catch the raw `ValueError` from an invalid `SlippageSide` string. Awaiting formal QA/Execution-Risk review. |
| TASK-009-02 | 90% | `src/backtest/execution_simulator.py` implemented: beta-weighted round-trip entry/exit using fill_model, LEG_FILL_MISMATCH detection, genuine integration with `evaluate_ack_guard` to delay exit when entry is still ACK_UNKNOWN-unresolved. Found and fixed a real bug during testing: the ack-guard delay check compared entry's static ack_status without accounting for whether reconciliation would already be complete by the planned exit time. 7 tests passed. Awaiting formal Quant Research/QA review. |
| TASK-009-03 | 90% | `src/backtest/replay_engine.py` implemented: reuses `generate_pair_signal_intents` unchanged, bounded FIFO day-cache (never holds unbounded decompressed days), fail-closed on missing archives. 5 tests passed including a dedicated bounded-cache test. Awaiting formal QA/Market Data review. |
| TASK-009-04 | 100% | `tests/test_sprint9_chaos.py` added: large data-gap NO_QUOTE, zero-liquidity zero-fill (no crash), simultaneous both-leg exit failure (NO_EXIT_FILL), invalid-side fail-closed (caught the ValueError bug above). Full suite: 238 passed, ruff clean. |
| TASK-009-05 | in progress | `scripts/run_sprint9_replay.py` written and run for real against real checksum-verified June-2023 tick data (no new downloads). Single-pair dry run (ARBUSDT/OPUSDT) already complete: 20 signals, 20 executed, 8/20 leg-fill-mismatch, 9 partial entry fills, net_pnl_quote=-147.12 (REALISTIC_NET_NEGATIVE) -- flips from Sprint 8's idealized +331.87bps for the same pair. Full 13-pair run in progress. |
| TASK-009-06 | 0% | BLOCKED until TASK-009-05 completes. |
