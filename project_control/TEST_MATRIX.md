...
| TASK-TSM-003 ERC dev run (DEVELOPMENT, no verdict): base (naive inverse-vol) vs ERC (correlation-aware, 90d covariance) across the robustness battery | integration | TSM | research | ran -> **BORDERLINE (first broad improvement)**: Sharpe 0.970 -> 1.039 (+0.069), maxDD 0.347 -> 0.326, turnover 0.457 -> 0.396 (lower). Better in all 3 sub-periods, every cost level, with/without funding, DD better in every cut; turnover drops (ERC mechanism). Misses strict letter on ONE cut (BTC-down Sharpe -0.003, noise). Criterion not relaxed; leading OOS candidate pending user integrity call. Cost = complexity for +0.069 dev Sharpe | sim |
