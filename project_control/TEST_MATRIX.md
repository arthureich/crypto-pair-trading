...
| Purged+embargoed walk-forward CV harness (TASK-ML-001): chronological non-overlapping folds; train strictly in the past; the leakage trap a naive past-split would allow (a past sample whose label window reaches into the test fold is purged); embargo removes what the pure purge would keep; all-legs-of-a-decision-time stay in one fold; fail-closed on mismatched lengths, label_end<decision, NaN/non-integer times, too-few-distinct-times, empty train after purge, invalid n_splits/embargo | unit | ML-1 | research | passed (15 tests, pure/deterministic, no ML dependency) | sim |
| Meta-labeling panel + filtered runners (TASK-ML-001): causal leg-level features (shift(1)-before-rolling; future bar does not change an earlier feature), hand-computed hold PnL/label sign, entry-cost accounting, per-(leg,interval) panel yields far more rows than entry-only; BOTH filtered runners (Option 1 entry-veto, Option 2 per-interval veto+renormalization) proven to reproduce the canonical incremental backtest exactly under an allow-all gate; veto removes a symbol from every resolved held set; fail-closed on missing column/duplicate rows | unit | ML-1 | research | passed (12 tests) | sim |
| Meta-labeling CV model/threshold selection (TASK-ML-001): purged folds + per-fold-span filtered evaluation + threshold sweep + min-kept-rebalances floor, dependency-injected model factory; fail-closed on no-candidate-meets-floor and empty grid/thresholds | unit | ML-1 | research | passed (3 tests, no XGBoost needed) | sim |
| Meta-labeling XGBoost CV selection real run (TASK-ML-001, DEVELOPMENT, no verdict): 30,140 leg-interval rows, 24-cell grid x 5 purged folds x 9 thresholds on the existing window | integration | ML-1 | research | ran (selected max_depth=3/n_est=100/lr=0.03/mcw=5, threshold=0.6, mean fold filtered PF=4.99 -- flagged as a RATIO-INFLATION MIRAGE: 2 folds PF ~11-12 on tiny net PnL, 3 of 5 folds worse than baseline; cautionary/negative dev signal). No promotion verdict -- gate blocked until new OOS | sim |
| Risk-based position sizing overlay (TASK-FC-II-001): inverse-vol weights favor the calmer leg and sum to 0.5/side, fail-safe to equal weight on NaN/zero/negative vol, empty side; causal leg vol (future bar does not change an earlier value); vol-target scale clamps to [min,max] and is neutral during warm-up; overlay's baseline leg reproduces the canonical K=5 net PnL exactly; summary reports baseline-vs-sized Sharpe/drawdown; fail-closed on invalid windows/scale bounds and empty summary | unit | FC-II | research | passed (11 tests) | sim |
| Risk position-sizing dev run (TASK-FC-II-001, DEVELOPMENT, no verdict): overlay vs equal-weight K=5 on the existing window, 3287 rebalances | integration | FC-II | research | ran (baseline Sharpe 0.993 -> sized 1.042, only +0.049 vs the +0.15 gate margin; max drawdown WORSE 1582 -> 1725 bps). Does not meet its own gate even in development -- cautionary/negative. PF-invariant by design (targets Sharpe/DD). No verdict -- gate blocked until OOS | sim |
| Forward paper-track OOS guard (ADR-0027): summarizes the fixed K=5 policy on post-cutoff bars and reports progress toward the trigger; FAIL-CLOSED if any bar at or before the 2026-05-31 dev cutoff leaks in (would contaminate the OOS claim); assemble concats monthly frames and guards; fail-closed on empty/missing columns/invalid trigger | unit | FC-II | research | passed (5 tests) | sim |
| Funding carry K=5 forward paper track real run (ADR-0027, MONITORING, no verdict): fixed K=5 on genuine OOS June 2026 (89 rebalances) | integration | FC-II | research/market_data | ran (OOS PF=0.7830, net=-300.4bps, hit=0.494 -- first unseen month is NET-NEGATIVE; the in-sample near-miss 1.0904 did not persist). One month is noisy and 411 short of the 500 trigger, so NOT a verdict, but a cautionary early read consistent with the meta-labeling/sizing dev signals | sim |
| Partial Spearman correlation (info_content, for TASK-FC-II-002): removes only the control's shared effect (control near-orthogonal -> partial ~= raw), NaN when feature is rank-collinear with control (no independent variation, FP tolerance) or n<2, drops NaN rows jointly | unit | FC-II | research | passed (4 tests) | sim |
| TASK-FC-II-002 basis diagnostic real run: 4 causal basis features (level/z-score/change/excess-funding) from the premium index already in the bars, vs 24h forward return, standard AND partial-over-funding info-content | integration | FC-II | research | ran (all 4 SEM_INFORMACAO on both the standard and the incremental-over-funding test, abs(rho)<0.013). Basis adds nothing beyond the funding carry -- closes. No download needed | sim |
| TASK-FC-II-003 short-horizon microstructure real run: 5 Family H book-imbalance features verbatim vs 1h/4h forward return (10-cell grid), sign-consistency across 3 sub-periods as the multiple-testing defense | integration | FC-II | research | ran (imbalance_price_divergence clears the threshold at 1h rho +0.0350 and 4h +0.0337, sign-consistent -- the project's first directional info hit in a new test; but the gross decile spread is ~1-2 bps/interval median, NEGATIVE mean, dwarfed by 6-12 bps cost at 1-4h turnover -> statistically real, economically negligible -> ABORT, like the z-score micro-reversion). No strategy, no OOS burned | sim |
| TASK-FC-II-004 Family E (Flow) real run: aggressor taker fraction (bars) + 3 long/short positioning ratios (metrics archives already on disk, previously unused) as causal z-scores vs 24h/4h forward return (10-cell grid) | integration | FC-II | research | ran (all 10 cells SEM_INFORMACAO, abs(rho)<0.011, slightly contrarian but noise). Flow carries no economically-relevant directional information -- Family E closes. Corrected an earlier premature "data exhausted" claim by actually testing the on-disk flow data | sim |
| Classic vol-targeted TSM (TASK-FC-II-005): unit-gross inverse-vol normalization favors the calmer leg and preserves signs; all-zero/NaN row -> flat; max-drawdown hand-computed; trend-sign follows trailing return; cost reduces net vs zero-cost; signal is causal (a future bar does not change an earlier rebalance); fail-closed on missing column/duplicates/invalid config/empty summary | unit | FC-II | research | passed (9 tests) | sim |
| TASK-FC-II-005 classic vol-targeted TSM dev run (DEVELOPMENT, no verdict): sign of 28d trailing return, size ~1/vol, unit gross, 5d rebalance, 6bps/leg, 20 symbols on the dev window | integration | FC-II | research | ran (L/S Sharpe 1.038 vs buy-hold -0.143; maxDD 0.35 vs 1.38; long-only Sharpe ~0). FIRST dev result to clear its own bar (better Sharpe AND lower DD) and not be economically dead -> REOPENS the price family; candidate for OOS. Caveats: in-sample, low buy-hold baseline. No verdict | sim |
| TASK-FC-II-006 TSM robustness decomposition (DEVELOPMENT descriptive, no verdict): sub-period / long-vs-short sleeve / BTC-regime split of the FC-II-005 result | integration | FC-II | research | ran -> BROAD: positive in all 3 sub-periods (Sharpe 1.71/0.61/1.02), BOTH legs contribute (long +0.67, short +0.75), works in BTC-up (Sharpe 1.00) AND BTC-down (1.13). Not a single-regime/short-only artifact; params are literature-fixed (not tuned). Most credible lead in the project; next is OOS + cost realism | sim |
| TASK-FC-II-007 TSM cost stress (DEVELOPMENT sensitivity, no verdict): sweep cost_bps_per_leg over a fixed grid with locked FC-II-005 params; report degradation curve + net-PnL breakeven | integration | FC-II | research | ran -> SURVIVES: breakeven 142 bps/leg (vs realistic 10-15); Sharpe 0.97 at 15bps, 0.70 at 50bps; turnover ~0.46/rebalance (28d signal is slow). Survives the most-likely killer | sim |
| Vol-targeted TSM with funding P&L (TASK-FC-II-008): include_funding flag adds perp funding over each hold (long pays when funding>0, short receives, reusing the leg_pnl_fracs convention); zero-funding matches the no-funding run; positive funding costs longs / pays shorts; fail-closed when funding columns missing | unit | FC-II | research | passed (3 new tests, 13 total in tsm_trend) | sim |
| TASK-FC-II-008 TSM-with-funding dev run (DEVELOPMENT, no verdict): funding P&L over each 5d hold added to the locked signal | integration | FC-II | research | ran -> SURVIVES funding: Sharpe 1.038 -> 0.970 (~7% haircut), net +1.27, baseline -0.143, cost breakeven 132 bps/leg (was 142). long-pays/short-receives nearly cancels. TSM has now survived EVERY in-sample stress (sub-period/leg/regime/cost/funding); only genuinely-new OOS remains | sim |
| TSM forward paper track (ADR-0027/0031, MONITORING): PRIMARY now the combined ERC+vol-target candidate (TASK-TSM-008); base TSM + buy-hold as reference; only post-cutoff rebalances count as OOS; vol-target overlay applied to the FULL ERC series then sliced (causal) | integration | TSM | research | ran -> 5 OOS rebalances (June 2026), pure noise (combined Sharpe 4.04, 95 to the 100 trigger). Lead-candidate OOS clock started; re-run monthly as data accrues | sim |
| TASK-ALT-008 Family B (range-vol shape) + Family C (Amihud) real run: Parkinson/Rogers-Satchell/close-location + Amihud/turnover/trade-size as causal bar-only features vs 24h/4h forward return (12-cell grid) | integration | ALT | research | ran (all 12 cells SEM_INFORMACAO, max abs(rho) 0.027 < 0.03). Range-vol magnitude highest but sub-threshold (vol-clustering, not directional); Amihud ~0.00; close-location weakly contrarian. Families B and C -> CONCLUIDA (public data). Closes the public-data family sweep -- only external-data families remain | sim |
| TASK-ALT-009 downloader pure parse (tidy_rows/build_panel): Coin Metrics community response -> tidy [asset,day,metric]; drops missing/empty; outer-merge on (asset,day); deterministic sort; all-empty skeleton | unit | ALT | research | passed (4 tests, fixture; no network mock, same precedent as S3 downloaders) | sim |
| TASK-ALT-009 Family G (On-Chain) real run on Coin Metrics community (ZERO cost): mvrv_z / active_addr_growth_z / tx_count_growth_z / exchange_netflow_z vs 1d/7d forward daily return (8-cell grid), daily causal shift(1) | integration | ALT | research | ran (all 8 cells SEM_INFORMACAO by locked rule). Fixed a real bug mid-run: synthesized open_time was in seconds not ms (us-resolution timestamp + astype pitfall) -> 0 obs/sub-period; fixed with unit-independent ms conversion. Near-miss exchange_netflow_z@7d (rho -0.0346, theory-coherent) clears magnitude but sign-consistency fails + BTC/ETH-only. First external family, zero spend; free proxies null | sim |
| TASK-ALT-010 downloader/diagnostic pure parse (select_markets/tidy_funding/cross_venue_stats): Coinalyze future-markets -> USDT-perp filter; funding-rate-history -> tidy [asset,day,venue,funding]; cross-venue std/range/mean requiring >=3 venues | unit | ALT | research | passed (4 tests, fixtures; no network mock) | sim |
| TASK-TSM-001 regime filter (`_trend_strength_regime` + `regime_filter` flag): binary/causal, OFF during median warm-up, flags high-vs-low strength, default-OFF leaves base TSM unchanged, book flat when median undefined | unit | TSM | research | passed (4 new tests, 17 in tsm_trend) | sim |
| TASK-TSM-001 regime-filter dev run (DEVELOPMENT, no verdict): base vs filtered TSM across the robustness battery (headline, 3 sub-periods, BTC up/down, cost grid, funding on/off, exposure) | integration | TSM | research | ran -> **REJECTED**: Sharpe 0.970 -> 0.949 (worse), maxDD 0.347 -> 0.321 (better), book live 42%. Opposing sub-period (helps only 2024-06/2025-05) and BTC-regime (helps down, hurts up) effects = false-positive signature. Only turnover reduction is real (wins at unrealistic 30-60 bps). Hypothesis closed negative; proceed to Line 2 | sim |
| TASK-TSM-002 conviction sizing (`_signal_raw_weights` + `conviction_sizing` flag): base is direction-only (equal \|weight\| at equal vol), conviction tilts to the stronger trend, preserves sign, default-OFF matches base, changes the book but keeps the unit-gross sleeves invariant | unit | TSM | research | passed (4 new tests, 21 in tsm_trend) | sim |
| TASK-TSM-002 conviction-sizing dev run (DEVELOPMENT, no verdict): base vs conviction (weight ~ trailing/vol) across the robustness battery | integration | TSM | research | ran -> **REJECTED**: Sharpe 0.970 -> 0.888 (worse), maxDD 0.347 -> 0.390 (worse), turnover 0.457 -> 0.566 (higher). Worse on all headline metrics + recent sub-period + BTC-up + every cost level. Knob-free linear strength-scaling churns more (the literature's turnover cut needs a saturating response with tuning constants, excluded). Closed negative; proceed to Line 3 (portfolio construction) | sim |
| TASK-TSM-003 ERC (`erc_weights` coordinate descent + `_erc_reweight` + `portfolio_erc` flag): ERC reduces to inverse-vol when uncorrelated, equalizes risk contributions, symmetric equal-var -> equal weight, degenerate sizes, reweight preserves sleeve gross/signs + base fallback on warm-up, default-OFF matches base | unit | TSM | research | passed (6 new tests, 27 in tsm_trend) | sim |
| TASK-TSM-003 ERC dev run (DEVELOPMENT, no verdict): base (naive inverse-vol) vs ERC (correlation-aware, 90d covariance) across the robustness battery | integration | TSM | research | ran -> **CARRIED FORWARD (first broad improvement)**: Sharpe 0.970 -> 1.039 (+0.069), maxDD 0.347 -> 0.326, turnover 0.457 -> 0.396 (lower). Better in all 3 sub-periods, every cost level, with/without funding, DD better in every cut; turnover drops (ERC mechanism). Misses strict letter on ONE cut (BTC-down Sharpe -0.003, noise). Criterion not relaxed; carried forward as leading OOS candidate (user-delegated). Cost = complexity for +0.069 dev Sharpe | sim |
| TASK-TSM-004 meta-labeling module (build_leg_panel/run_meta_labeled_cv + keep_mask): 6 causal features, binary profitability label, causal-span purge basis, feature causality, purged-CV mask 0/1, all-ones mask == base, frozen threshold 0.5 | unit | TSM | research | passed (7 new tests) | sim |
| TASK-TSM-004 meta-labeling filter dev run (DEVELOPMENT, no verdict; OUT-OF-FOLD comparison): GradientBoosting P(leg profitable) gate via purged walk-forward CV | integration | TSM | research | ran -> **REJECTED/CAUTIONARY**: base leg precision ~0.50; OOF Sharpe 0.784 -> 0.412 (worse), maxDD 0.347 -> 0.503 (worse), helped only 3/5 folds. ML-001 mirage replay -- fold-specific gains don't survive purged OOF. Line 4 closed; proceed to Line 5. Gate BLOCKED until OOS | sim |
| TASK-TSM-005 ensemble module (weekly_pnl / blend_diagnostic): calendar-week bucketing, length-mismatch/empty guards, equal-risk standardized blend beats like-Sharpe uncorrelated components ~sqrt(2)x, overlapping-weeks-only, negatively-correlated cuts drawdown | unit | TSM | research | passed (6 new tests) | sim |
| TASK-TSM-005 trend+carry ensemble dev run (DEVELOPMENT, no verdict): equal-risk 50/50 blend of TSM + funding-carry K=5 weekly streams | integration | TSM | research | ran -> **CARRIED FORWARD (candidate; carry-OOS-fragile)**: blend Sharpe 0.987 -> 1.510 (+0.523), corr -0.037, maxDD 4.90<5.88, beats TSM in all 3 sub-periods (clean criterion pass). BUT carry leg's own K=5 OOS month was negative -> inherits OOS fragility; weaker than ERC (pure-TSM). Strongest dev number; OOS-gated | sim |
| TASK-TSM-006 execution (reasoned closure, no build): TSM cost-insensitive (FC-II-007 breakeven 142 bps/leg) + Sprint 10 maker null -> execution optimization not justified | documentation | TSM | research | closed NOT-JUSTIFIED by existing evidence (ADR-0031 rule 6). Completes the 6-line TSM program | sim |
| TASK-ALT-011 DVOL downloader pure parse (tidy_dvol): Deribit DVOL OHLC rows -> [asset,day,dvol_close], lowercases asset to base_asset convention, empty skeleton | unit | ALT | research | passed (2 tests, fixture; no network mock) | sim |
| TASK-ALT-011 Family F options DVOL/VRP-as-predictor real run (free Deribit DVOL, ZERO cost): dvol_z / vrp_z / dvol_change_z / iv_rv_ratio_z (BTC+ETH) vs 7d/30d forward return (8-cell grid) | integration | ALT | research | ran -> **HIT (first external)**: `vrp_z@7d` TEM_INFORMACAO (rho +0.087, positive & sign-consistent in all 3 sub-periods +0.088/+0.046/+0.110, theory-coherent). vrp@30d bigger full rho but flips in period 1 -> correctly SEM. BTC/ETH-only (2-asset pooled). Economic check is the pre-registered next step; not an options-book pivot | sim |
| TASK-ALT-012 vrp_timing module (compute_vrp_z/backtest/summary): unit-gross+flat, max-drawdown, causal vrp_z, signs follow signal, long-only never shorts, cost reduces net, causal (future bar doesn't change earlier rebalance), fail-closed | unit | ALT | research | passed (8 new tests) | sim |
| TASK-ALT-012 VRP-timing strategy dev run (DEVELOPMENT, no verdict): long/short sign(vrp_z) weekly BTC/ETH vs buy-hold + robustness battery | integration | ALT | research | ran -> **NAO_PASSA (dev)**: Sharpe 0.493 vs buy-hold 0.349 (better) but maxDD 0.787 vs 0.774 (worse) + inconsistent sub-periods (neg 2024-25). Naive standalone modest/drawdown-heavy (decile spread was overlapping-inflated). Long-only secondary better (0.601/0.500) but descriptive-only. VRP best as feature/overlay. OOS-gated | sim |
| TASK-ALT-013 VRP-as-overlay on TSM dev run (DEVELOPMENT, no verdict): equal-risk blend of TSM + VRP-timing streams (reuses tsm_ensemble.blend_diagnostic) | integration | ALT | research | ran -> **NAO_PASSA (dev)**: blend Sharpe 0.966 < TSM 0.991 (worse), VRP sleeve 0.493, corr +0.18 (positive). Weak positively-correlated sleeve drags Sharpe down; VRP -0.48 in 2024-25. Closes free VRP exploration (real signal, no free monetization). Skew/surface + Angle A = user decisions. OOS-gated | sim |
| TASK-TSM-007 vol-target overlay (vol_target_scale/apply_vol_target): causal (future return doesn't change earlier scale), down-weights high-vol / up-weights low-vol, respects cap + non-negative, warm-up scale=1, scales returns, fail-closed | unit | TSM | research | passed (6 new tests) | sim |
| TASK-TSM-007 volatility-targeted TSM dev run (DEVELOPMENT, no verdict): managed-vol overlay (Moreira-Muir) on the base TSM return stream + robustness battery | integration | TSM | research | ran -> **CARRIED FORWARD (cleanest candidate)**: Sharpe 0.970 -> 1.107 (+0.137), maxDD 0.347 -> 0.329 (better), better in all 3 sub-periods, both BTC regimes, every cost level. Passes more cleanly than ERC. 3rd TSM OOS candidate (pure-TSM). OOS-gated | sim |
| TASK-TSM-008 combined ERC + vol-targeting dev run (DEVELOPMENT, no verdict): composes portfolio_erc + vol_target overlay; 4-variant comparison (base/ERC/vol/combined) + robustness battery | integration | TSM | research | ran -> **LEAD OOS CANDIDATE**: Sharpe base 0.970 / ERC 1.039 / vol 1.107 / **combined 1.183** (+0.076 over best single), maxDD 0.309 (lowest), best in all 3 sub-periods + both BTC regimes + every cost level. Orthogonal overlays stack complementarily; best TSM config. Supersedes standalone ERC/vol-target. OOS-gated | sim |
| TASK-TSM-009 out-of-universe generalization test: FIXED combined config on a DIFFERENT 10-alt universe (klines+funding cached; download-availability filtered), zero re-tune | integration | TSM | research | ran -> **CORE generalizes, OVERLAYS don't**: base 0.577 / combined 0.335 / buy-hold -0.429. Core trend edge beats buy-hold in every sub-period on a fresh universe (generalizes -> not an artifact of the 20). BUT combined 0.335 < base 0.577 (opposite of orig 20) -> ERC+vol-target overlays are partly universe-specific/overfit. Raises confidence in base, tempers it in combined. Not a live promotion | sim |
| TASK-TSM-010 multi-universe generalization: FIXED base TSM (zero re-tune) across 6 thematic USDM-perp universes; per-symbol resilient download; coverage-gated | integration | TSM | research | ran -> **STRONG breadth**: base Sharpe > 0 AND > buy-hold in 6/6 universes (large_cap 0.99, gaming 1.00, defi 0.83, mid_alt_l1 0.65, mid_tier_ref 0.58, old_guard 0.46; buy-hold negative in 4). Strongest anti-overfit evidence (out-of-sample in the asset dimension). Combined beats base in only 2/6 -> base is the robust core. AI/memecoins untestable (no 3y history). Not a live promotion | sim |
| TASK-TSM-011 cross-exchange generalization: FIXED base TSM (zero re-tune), same 20 symbols/code, Bybit v5 (paginated klines+funding, as-of merge) vs Binance | integration | TSM | research | ran -> **GENERALIZES (venue-independent)**: Bybit Sharpe 0.971 vs Binance 0.970, maxDD/turnover/net/sub-periods near-identical. Rules out Binance data/microstructure artifact. Caveat: arbitraged same-symbol prices -> similarity expected; cross-universe is stronger diversity. Near-identical match validates the Bybit pipeline. OKX follow-up; Hyperliquid dropped. Not a live promotion | sim |
| TASK-ALT-010 cross-venue funding dispersion real run on Coinalyze (ZERO cost): disp/range/mean of daily funding across {Binance,Bybit,OKX,Huobi,BitMEX} (>=3 venues/day) vs 1d/3d forward return (6-cell grid); 429 handled with backoff | integration | ALT | research | ran (95 markets, 103,325 rows; all 6 cells SEM_INFORMACAO by locked rule). Strongest external near-miss: disp/range@3d clear magnitude (+0.040) but one-regime mirage (middle sub-period +0.10, recent negative -> sign-consistency fails); @1d sign-consistent but sub-threshold. Mean control ~0. Exhausts the free-tier external avenue (on-chain + cross-venue both null) | sim |
