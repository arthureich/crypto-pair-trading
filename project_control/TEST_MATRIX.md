...
| TSM forward paper track (ADR-0027, MONITORING): fixed TSM (with funding) counting only post-cutoff rebalances as OOS; pre-cutoff bars form the causal 28d signal (lookback, not test) | integration | FC-II | research | ran -> 5 OOS rebalances so far (June 2026), too few to be anything but noise (95 to the 100-rebalance trigger). Mechanism started the OOS clock; re-run monthly as data accrues | sim |
