...
| Sprint 9 executable backtest uses aggressive MARKET_IOC on both legs of both entry and exit -- the most expensive execution style tested; `simulate_limit_fill` exists and is tested but is never called by `execution_simulator.py`/the real runner | High | Execution / Risk Agent | 0/13 pairs profitable under this assumption does not prove the strategy has zero edge; test a passive/maker execution variant before drawing that conclusion (explicit Execution/Risk Agent recommendation, reports/backtest_executable_v1.md) | Open |
...
| `simulate_limit_fill` hardcodes `slippage_bps=0.0` regardless of reference price, an inconsistent metric definition versus MARKET_IOC orders | Low | QA Agent | Not a hidden-PnL bug (average_price/filled_quantity are still correct); fix before building slippage-by-order-type analysis on top of this path | Open |
